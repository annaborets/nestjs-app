...
export class OrdersService {
    ...
    async createOrder(dto: CreateOrderDto): Promise<Order> {
        ...
        if (existingOrder) {
            console.log(`Idempotency key ${dto.idempotencyKey} already exists, returning existing order`);
        ...
        try {
            let totalPrice = 0;
            const orderItemsToCreate: Array<{ product: Product; quantity: number; price: number }> = [];
            ...
            console.log(`Order ${savedOrder.id} created successfully with idempotency key ${dto.idempotencyKey}`);
